 ...
 * */
let result = null;
export function getTreeItem(data: any[], key?: string | number,  keylabel ?: string = "key"): any {
  data.map((item) => {
    if (item[keylabel] === key) {
      result = item;
    } else {
      if (item.children && item.children.length) {
        getTreeItem(item.children, key, keylabel);
      }
    }
  });
  return result;
 ...
 * */
const treeAll: any[] = [];
export function getTreeAll(data: any[], key: string = 'key'): any[] {
  data.map((item) => {
    treeAll.push(item[key]);
    if (item.children && item.children.length) {
      getTreeAll(item.children, key);
    }
  });
  return treeAll;
}
